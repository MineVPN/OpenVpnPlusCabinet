...
$error    = '';
$attempts = sys_get_temp_dir() . '/ovpnplus_login_' . md5($_SERVER['REMOTE_ADDR'] ?? 'unknown');
...
function ovp_login_allowed(string $file): bool {
...
function ovp_login_failed(string $file): void {
    $fp = @fopen($file, 'c+');
    if (!$fp) return;
    flock($fp, LOCK_EX);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['password'])) {
    if (!ovp_login_allowed($attempts)) {
        $error = 'Слишком много попыток. Подождите пять минут.';
        ovp_log('WARN', 'Вход заблокирован: превышен лимит попыток');
    } elseif (ovp_check_password((string) $_POST['password'])) {
        session_regenerate_id(true);
        $_SESSION['authenticated'] = true;
        $_SESSION['login_time']    = time();
        unset($_SESSION['csrf']);
        @unlink($attempts);
        ovp_log('OK', 'Вход в панель');
        header('Location: index.php');
        exit();
    } else {
        ovp_login_failed($attempts);
        ovp_log('WARN', 'Неверный пароль при входе');
...
<div class="auth">
  <div class="auth__card">
    ...
    <div class="auth__title">OpenVPN+</div>
