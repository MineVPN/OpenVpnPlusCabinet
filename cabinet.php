<?php
/**
 * OpenVPN+ — каркас панели: боковое меню + рабочая область.
 *
...
$invalid = ovp_session_invalid_reason();
if ($invalid !== '') {
    ovp_session_kill($invalid);
}
...
// is_string обязателен: ?menu[]=x дал бы «Array to string conversion»,
// а warning до первого вывода ломает все последующие header().
 ...
 *
 * ЗАЧЕМ: обработчики POST внутри страниц заканчиваются
 * header('Location: …'). Если сначала вывести шапку и меню, а страницу
 * подключить после, то заголовки уже отправлены и редирект молча не
 * сработает — при выключенном display_errors пользователь получит
 * страницу с пустым содержимым, а повторное F5 выполнит операцию заново
 * (перезапуск туннеля, удаление конфига).
 *
 * Полагаться на output_buffering из php.ini нельзя: он отключается
 * одной строкой в конфиге и не гарантирован на всех хостингах.
 */
ob_start();
...
// Иконка подключения: канал с узлом посередине — два сегмента и точка связи.
...
// Обход: поток раздваивается на два пути со стрелками — часть трафика
// идёт мимо туннеля.
...
<title><?= htmlspecialchars($pages[$menu]['title']) ?> — OpenVPN+</title>
...
<?php /* Загружается синхронно в head: страницы вызывают свои ovpInit*()
         инлайново в теле документа, и к этому моменту функции обязаны
         уже существовать. В конце body они бы вызывались раньше загрузки. */ ?>
...
<div class="shell">
  <aside class="side">
    <a class="side__brand" href="cabinet.php">
      ...
      <span>
        <span class="side__name">OpenVPN+</span><br>
    ...
    <div class="side__foot">
      <a class="navlink navlink--exit" href="logout.php?t=<?= htmlspecialchars(ovp_logout_token(), ENT_QUOTES) ?>"><?= $icoExit ?><span>Выйти</span></a>
    </div>
  </aside>
