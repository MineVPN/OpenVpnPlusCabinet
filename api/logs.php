<?php
/**
 * OVPNPlus — журнал в JSON для страницы «События».
 *
 * Разбирает строки общего лога вида
 *   [2026-07-24 15:30:12] [OK] [панель] Текст
 * и отдаёт готовыми полями, чтобы фронт не занимался разбором.
 *
 * GET:
 *   lines — 50..3000
...
// Снимаем блокировку сессии: журнал опрашивается каждые 5 секунд, и держать
// её всё это время значит блокировать остальные вкладки панели.
session_write_close();
...
$rows = [];
foreach (ovp_tail(OVP_LOG_FILE, $lines) as $raw) {
