 ...
 */
/**
 * @param bool $quiet Не писать в журнал при неудаче. Для необязательных
 *   операций вроде reset-failed: при первой установке юнит ещё не загружен
 *   systemd, команда честно возвращает единицу, и запись об этом только
 *   пугает владельца сообщением об ошибке там, где ничего не сломалось.
 */
function ovp_sudo(string $cmd, bool $quiet = false): bool {
    $out  = [];
    $code = 0;
...
/**
 * Состояние второго VPN в виде, готовом для показа.
 *
 * ЗАЧЕМ ОТДЕЛЬНОЙ ФУНКЦИЕЙ: этот расчёт нужен в двух местах — при отрисовке
 * страницы и в api/status.php, который опрашивается раз в несколько секунд.
 * Если считать в каждом месте по-своему, плашка на странице и плашка после
 * автообновления однажды разойдутся.
 *
 * @return array{kind:string, text:string, note:string, hasConfig:bool,
 *               up:bool, connection:string, sig:string}
 *   sig — признак «материального» состояния: при его смене меняется и набор
 *   доступных кнопок, а их живьём не подменить, нужна перерисовка страницы.
 */
function ovp_state_view(): array {
    ...
    if (!$hasConfig) {
        $kind = 'off';
        $text = 'Не настроено';
        $note = 'Сейчас сервер работает как обычный OpenVPN. Клиенты выходят в интернет '
              . 'с адреса самого сервера. Загрузите впн конфиг, чтобы включить двойной впн.';
    } elseif ($up && $health['known'] && $health['bypass']) {
        $kind = 'warn';
        $text = 'Обход';
        $note = 'Второй впн не отвечает, поэтому клиенты временно выходят напрямую через этот сервер. '
              . 'Сайты сейчас видят адрес этого сервера. Мониторинг продолжает попытки восстановить '
              . 'туннель — когда второй впн вернётся, трафик пойдёт через него автоматически.';
    } elseif ($up && $health['known'] && !$health['alive']) {
        $kind = 'err';
        $text = 'Проверяется';
        $note = 'Интерфейс поднят, но связь через туннель пока не подтверждена. Мониторинг разбирается.';
    } elseif ($up) {
        $kind = 'ok';
        $text = 'Подключено';
        $note = $health['known']
            ? 'Трафик клиентов идёт через второй впн.'
            : 'Интерфейс поднят. Демон мониторинга не отвечает, поэтому подтвердить прохождение '
              . 'трафика нельзя — проверьте: systemctl status ovpn-healthcheck';
    } else {
        $kind = 'err';
        $text = 'Нет связи';
        $note = $killswitch
            ? 'Впн конфиг загружен, но соединение не установлено. Интернета у клиентов сейчас нет — '
              . 'так работает Kill Switch.'
            : 'Впн конфиг загружен, но соединение не установлено. Клиенты сейчас работают напрямую '
              . 'через этот сервер.';
    }
    ...
    return [
        ...
        'hasConfig'  => $hasConfig,
        'up'         => $up,
        'connection' => $up ? 'активно' : 'нет',
        'sig'        => ($hasConfig ? '1' : '0') . ($up ? '1' : '0'),
    ];
}
