<?php
// PHP-логика остается без изменений
session_start(); 
...
<div class="flex flex-col gap-8">
    ...
    <div class="glassmorphism rounded-2xl p-6">
        <div class="flex flex-col sm:flex-row items-center gap-4">
            <label for="targetAddress" class="text-lg font-medium text-white whitespace-nowrap">IP / Адрес:</label>
            <input type="text" id="targetAddress" placeholder="Введите адрес, например, 8.8.8.8" value="8.8.8.8" class="flex-grow w-full bg-slate-700/50 border border-slate-600 rounded-lg p-3 text-white placeholder-slate-400 focus:ring-2 focus:ring-orange-500 focus:outline-none transition">
    ...
    <div class="glassmorphism rounded-2xl p-6">
        <div class="grid grid-cols-2 md:grid-cols-4 gap-6 text-center">
            ...
            <div class="bg-slate-800/50 p-4 rounded-lg">
                <p class="text-sm font-medium text-slate-400">Мин. / мс</p>
                <p class="text-3xl font-semibold text-sky-400 mt-1"><span id="minPing">-</span></p>
            </div>
            <div class="bg-slate-800/50 p-4 rounded-lg">
                <p class="text-sm font-medium text-slate-400">Сред. / мс</p>
                <p class="text-3xl font-semibold text-sky-400 mt-1"><span id="avgPing">-</span></p>
            </div>
             <div class="bg-slate-800/50 p-4 rounded-lg">
                <p class="text-sm font-medium text-slate-400">Макс. / мс</p>
                <p class="text-3xl font-semibold text-sky-400 mt-1"><span id="maxPing">-</span></p>
            </div>
            <div class="bg-slate-800/50 p-4 rounded-lg">
                <p class="text-sm font-medium text-slate-400">Последний / мс</p>
                <p class="text-3xl font-semibold text-white mt-1"><span id="lastPing">-</span></p>
            </div>
...
<script>
    ...
    document.getElementById("startButton").addEventListener("click", function() {
        ...
        if (targetAddress.trim() === "") {
            // Заменяем alert на нашу новую функцию уведомлений
        ...
        xhr.onreadystatechange = function() {
            if (xhr.readyState == 4) {
                ...
                if (xhr.status == 200 && xhr.responseText.indexOf("NO PING") == -1) {
                    ...
                    logEntry.textContent = `[${now}] Ответ от ${targetAddress}: время=${ping.toFixed(2)} мс`;
                    ...
                    ping = NaN;
                    var logEntry = document.createElement('p');
                    logEntry.textContent = `[${now}] Ответ от ${targetAddress}: NO PING`;
